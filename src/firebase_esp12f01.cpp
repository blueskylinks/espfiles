...
UserAuth user_auth("AIzaSyDpplKB_Cko3-k4XZczdcE9v_lJGmc2EWo", "chaitanya.csd@gmail.com", "chai1234");
...
void setup()
{
    ...
    // Step 10
    Database.url("https://esp-02-default-rtdb.firebaseio.com");
}
...
void loop()
{
    ...
    if (app.ready() && !onetimeTest)
    {
        ...
        Database.set<String>(async_client1, "/example/BareMinimum/data/set1", "abc", processData, "RealtimeDatabase_SetTask");

        // Async call with AsyncResult for returning result.
        Database.set<bool>(async_client1, "/example/BareMinimum/data/set2", true, dbResult);

        // Realtime Database get value.
        // ============================

        // Async call with callback function
        Database.get(async_client1, "/example/BareMinimum/data/set1", processData, false, "RealtimeDatabase_GetTask");

        // Async call with AsyncResult for returning result.
        Database.get(async_client1, "/example/BareMinimum/data/set2", dbResult, false);

        // Await call which waits until the result was received.
        String value = Database.get<String>(async_client2, "/example/BareMinimum/data/set3");
