...
void readSchedule(int index, int &hour, int &min, int &duration)
{
    ...
    {
    case 0:
        baseAddr = SCHED1_ADDR_HOUR;
        break;
    case 1:
        baseAddr = SCHED2_ADDR_HOUR;
        break;
    case 2:
        baseAddr = SCHED3_ADDR_HOUR;
        break;
    default:
        hour = -1;
        min = -1;
...
void setup()
{
    ...
    if (digitalRead(input1) == LOW)
    {
        ...
        if (new_duration < 5 || new_duration > 100)
            new_duration = 30;
...
void loop()
{
    if (!scheduleChecked && wifiCheckCounter < 240)
    {
    ...
    if (millis() - lastFirebaseFetchTime >= firebaseFetchInterval)
    {
        if (WiFi.status() != WL_CONNECTED)
            connectToWiFi();
        getScheduleFromFirebase();
    ...
    int matchedSchedule = findMatchingSchedule(currentHour, currentMin);
    ...
    if (motor_status == 0 && matchedSchedule != -1 && !scheduleCompleted[matchedSchedule])
    {
        int h, m, d;
    ...
    if (motor_status == 1)
    {
        ...
        if (ot_sensorstatus == 0)
        {
            ...
            if (ot_sensorcount >= 5)
            {
                ...
                postRunToFirebase(durationToPost);
            }
        }
        else
        {
